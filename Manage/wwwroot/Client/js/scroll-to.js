...
$('.scroll-to').on('click', function (e) {
    const $section = $("#section-to").length > 0 ? $("#section-to") : $("#g-banner-section");

    const margin = parseFloat($(`#${$(this).data("target")}`).css("marginTop"))
    const $nav_height = parseFloat($('.g-nav__main').css("height"))
    const banner_height = parseFloat($section.css("height"))
    ...
    $(window).stop().animate({
        scrollY: margin + $nav_height + banner_height - 150
    }, 1000, 'linear');
});
// }
